...
app.innerHTML = `
  <div class="app-shell">
    <header class="topbar">
      <div class="brand"><strong>EvoSpore</strong><span>v0.1</span></div>
...
function updateStats(s: WorldSnapshot): void {
  ...
  statusEl.textContent = `${st.speedMode === 'deep' ? 'Deep Time' : st.speedMode} · ${st.thermalSafe ? 'thermal guard on' : 'thermal guard off'} · autosave every minute`;
}
...
worker.onmessage = async (event: MessageEvent<WorkerToMain>) => {
  ...
  if (message.type === 'ready') statusEl.textContent = 'Pond online.';
    ...
    snapshot = message.snapshot; renderer.setSnapshot(message.snapshot); updateStats(message.snapshot);
    ...
    try { await saveCheckpoint(message.checkpoint); if (message.reason === 'manual') showToast('World checkpoint saved.'); }
...
resetButton.addEventListener('click', async () => {
  if (!confirm('Reset this evolutionary world? The current autosave will be deleted.')) return;
  await clearCheckpoint().catch(() => undefined); selectedId = null; updateInspector(null); post({ type: 'resetWorld' }); showToast('New founder world created.');
});
...
async function boot(): Promise<void> {
  ...
  try {
    ...
    if (checkpoint) { post({ type: 'init', checkpoint }); showToast('Resumed saved ecosystem.'); }
    else post({ type: 'init' });
